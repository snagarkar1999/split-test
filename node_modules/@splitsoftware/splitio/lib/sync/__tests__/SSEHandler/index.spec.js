...
(0, _tape.default)('SSEHandler', function (t) {
  ...
  t.test('`handlerMessage` for update notifications (NotificationProcessor)', function (assert) {
    var _pushEmitter$emit$las, _context, _pushEmitter$emit$las2, _context2, _pushEmitter$emit$las3, _context3, _pushEmitter$emit$las4, _context4;
    ...
    assert.true((_pushEmitter$emit$las = pushEmitter.emit.lastCall).calledWithExactly.apply(_pushEmitter$emit$las, (0, _concat.default)(_context = [_constants.SPLIT_UPDATE]).call(_context, (0, _toConsumableArray2.default)(expectedParams))), 'must emit SPLIT_UPDATE with the message change number');
    expectedParams = [1457552650000, 'whitelist', 'not_allowed'];
    sseHandler.handleMessage(_messageSPLIT_KILL.default);
    assert.true((_pushEmitter$emit$las2 = pushEmitter.emit.lastCall).calledWithExactly.apply(_pushEmitter$emit$las2, (0, _concat.default)(_context2 = [_constants.SPLIT_KILL]).call(_context2, (0, _toConsumableArray2.default)(expectedParams))), 'must emit SPLIT_KILL with the message change number, split name and default treatment');
    expectedParams = [1457552640000, 'splitters'];
    sseHandler.handleMessage(_messageSEGMENT_UPDATE.default);
    assert.true((_pushEmitter$emit$las3 = pushEmitter.emit.lastCall).calledWithExactly.apply(_pushEmitter$emit$las3, (0, _concat.default)(_context3 = [_constants.SEGMENT_UPDATE]).call(_context3, (0, _toConsumableArray2.default)(expectedParams))), 'must emit SEGMENT_UPDATE with the message change number and segment name');
    expectedParams = [{
    ...
    assert.true((_pushEmitter$emit$las4 = pushEmitter.emit.lastCall).calledWithExactly.apply(_pushEmitter$emit$las4, (0, _concat.default)(_context4 = [_constants.MY_SEGMENTS_UPDATE]).call(_context4, (0, _toConsumableArray2.default)(expectedParams))), 'must emit MY_SEGMENTS_UPDATE with the message parsed data and channel');
    assert.end();
  });
