...
(0, _tape.default)('defaultMapper', function (assert) {
  var initTimestamp = (0, _now.default)();
  ...
  assert.true(initTimestamp <= defaultEvent.timestamp && defaultEvent.timestamp <= (0, _now.default)());
  assert.end();
});
var sdkOptions = {
...
function customMapper(model, defaultEvent) {
  return _objectSpread({}, defaultEvent, {
    properties: _objectSpread({}, defaultEvent.properties, {
      someProp: 'someProp'
    })
  });
...
(0, _tape.default)('GaToSplit', function (assert) {
  ...
  assert.deepEqual(event, _objectSpread({}, expectedDefaultEvent, {
  ...
  assert.deepEqual(event, _objectSpread({}, customMapper(modelSample, (0, _GaToSplit.defaultMapper)(modelSample)), {
  ...
  window.ga('send', hitSample);
  event = fakeStorage.events.track.lastCall.args[0];
  assert.deepEqual(event, _objectSpread({}, customMapper2(modelSample, (0, _GaToSplit.defaultMapper)(modelSample)), {
    key: customIdentities[0].key,
    trafficTypeName: customIdentities[0].trafficType,
    timestamp: event.timestamp
