...
var log = (0, _logger.default)('splitio-ga-to-split');
/**
...
function validateIdentities(identities) {
  if (!(0, _isArray.default)(identities)) return []; // Remove duplicated identities

  var uniqueIdentities = (0, _lang.unicAsStrings)(identities); // Filter based on rum-agent identities validator

  return (0, _filter.default)(uniqueIdentities).call(uniqueIdentities, function (identity) {
    if (!identity) return false;
    var maybeKey = identity.key;
    var maybeTT = identity.trafficType;
    if (!(0, _lang.isString)(maybeKey) && !(0, _lang.isFinite)(maybeKey)) return false;
...
function validateEventData(eventData) {
  if (!(0, _inputValidation.validateEvent)(eventData.eventTypeId, 'splitio-ga-to-split:mapper')) return false;
  if ((0, _inputValidation.validateEventValue)(eventData.value, 'splitio-ga-to-split:mapper') === false) return false;

  var _validateEventPropert = (0, _inputValidation.validateEventProperties)(eventData.properties, 'splitio-ga-to-split:mapper'),
      properties = _validateEventPropert.properties;

  if (properties === false) return false;
  if (eventData.timestamp && !(0, _lang.isFinite)(eventData.timestamp)) return false;
  if (eventData.key && (0, _inputValidation.validateKey)(eventData.key, 'splitio-ga-to-split:mapper') === false) return false;
  if (eventData.trafficTypeName && (0, _inputValidation.validateTrafficType)(eventData.trafficTypeName, 'splitio-ga-to-split:mapper') === false) return false;
  return true;
}
...
function fixEventTypeId(eventTypeId) {
  ...
  var truncated = (0, _slice.default)(fixed).call(fixed, 0, 80);
...
function GaToSplit(sdkOptions, storage, coreSettings) {
  ...
  function SplitTracker(tracker, pluginOptions) {
    ...
    var opts = (0, _assign.default)({}, defaultOptions, sdkOptions, pluginOptions);
    ...
    tracker.set('sendHitTask', function (model) {
      ...
      try {
        if ((0, _filter.default)(opts) && !(0, _filter.default)(opts).call(opts, model)) return;
      } catch (err) {
      ...
      if (opts.prefix) eventData.eventTypeId = (0, _concat.default)(_context = "".concat(opts.prefix, ".")).call(_context, eventData.eventTypeId);
      ...
      if (eventData.key && eventData.trafficTypeName) {
        ...
        (0, _forEach.default)(_context2 = opts.identities).call(_context2, function (identity) {
          var event = (0, _assign.default)({
            key: identity.key,
