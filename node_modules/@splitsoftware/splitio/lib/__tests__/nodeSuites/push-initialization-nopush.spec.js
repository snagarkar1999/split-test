...
function testInitializationFail(fetchMock, assert, fallbackToPolling) {
  ...
  if (fallbackToPolling) {
    fetchMock.getOnce(settings.url('/splitChanges?since=1457552620999'), function () {
      ...
      var lapse = (0, _now.default)() - start;
      assert.true((0, _utils.nearlyEqual)(lapse, 0), 'polling (first fetch)');
      return {
        status: 200,
  ...
  fetchMock.getOnce(settings.url('/splitChanges?since=1457552620999'), function () {
    assert.true(ready, 'client ready');
    var lapse = (0, _now.default)() - start;
    assert.true((0, _utils.nearlyEqual)(lapse, settings.scheduler.featuresRefreshRate), 'polling (second fetch)');
...
function testNoEventSource(fetchMock, assert) {
  assert.plan(3);
  (0, _node.__setEventSource)(undefined);
