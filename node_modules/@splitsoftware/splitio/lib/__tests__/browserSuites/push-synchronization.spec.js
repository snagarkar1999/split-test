...
function testSynchronization(fetchMock, assert) {
  ...
  (0, _eventSourceMock.setMockListener)(function (eventSourceInstance) {
    ...
    (0, _setTimeout2.default)(function () {
      ...
      client.once(client.Event.SDK_UPDATE, function () {
        ...
        assert.true((0, _utils.nearlyEqual)(lapse, MILLIS_FIRST_SPLIT_UPDATE_EVENT), 'SDK_UPDATE due to SPLIT_UPDATE event');
    ...
    (0, _setTimeout2.default)(function () {
      ...
      client.once(client.Event.SDK_UPDATE, function () {
        var lapse = (0, _now.default)() - start;
        assert.true((0, _utils.nearlyEqual)(lapse, MILLIS_MYSEGMENT_UPDATE_EVENT), 'SDK_UPDATE due to MY_SEGMENTS_UPDATE event');
    ...
    (0, _setTimeout2.default)(function () {
      ...
      client.once(client.Event.SDK_UPDATE, function () {
        var lapse = (0, _now.default)() - start;
        assert.true((0, _utils.nearlyEqual)(lapse, MILLIS_SPLIT_KILL_EVENT), 'SDK_UPDATE due to SPLIT_KILL event');
      ...
      (0, _eventSourceMock.setMockListener)(function (eventSourceInstance) {
        ...
        (0, _setTimeout2.default)(function () {
          ...
          otherClient.once(otherClient.Event.SDK_UPDATE, function () {
            var lapse = (0, _now.default)() - start;
            assert.true((0, _utils.nearlyEqual)(lapse, MILLIS_MY_SEGMENTS_UPDATE_WITH_PAYLOAD), 'SDK_UPDATE due to MY_SEGMENTS_UPDATE event (with payload)');
            assert.equal(otherClient.getTreatment('qc_team'), 'yes', 'evaluation with updated MySegments list (shared client)'); // destroy shared client and then main client
  ...
  fetchMock.getOnce(settings.url((0, _concat.default)(_context3 = "/auth?users=".concat(encodeURIComponent(userKey), "&users=")).call(_context3, encodeURIComponent(otherUserKey))), function (url, opts) {
    if (!opts.headers['Authorization']) assert.fail('`/auth` request must include `Authorization` header');
  ...
  fetchMock.getOnce(settings.url('/splitChanges?since=1457552620999'), function () {
    var lapse = (0, _now.default)() - start;
    assert.true((0, _utils.nearlyEqual)(lapse, MILLIS_SSE_OPEN), 'sync after SSE connection is opened');
    return {
      status: 200,
  ...
  fetchMock.getOnce(settings.url('/splitChanges?since=1457552650000'), function () {
    var lapse = (0, _now.default)() - start;
    assert.true((0, _utils.nearlyEqual)(lapse, MILLIS_SECOND_SSE_OPEN), 'sync after second SSE connection is opened');
    return {
      status: 200,
      body: {
        splits: [],
