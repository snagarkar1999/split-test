"use strict";

var _interopRequireWildcard = require("@babel/runtime-corejs3/helpers/interopRequireWildcard");

var _interopRequireDefault = require("@babel/runtime-corejs3/helpers/interopRequireDefault");

var _Object$defineProperty = require("@babel/runtime-corejs3/core-js-stable/object/define-property");

require("core-js/modules/es.regexp.constructor");

require("core-js/modules/es.regexp.exec");

require("core-js/modules/es.regexp.to-string");

_Object$defineProperty(exports, "__esModule", {
  value: true
});

exports.testFallbacking = testFallbacking;

var _now = _interopRequireDefault(require("@babel/runtime-corejs3/core-js-stable/date/now"));

var _setTimeout2 = _interopRequireDefault(require("@babel/runtime-corejs3/core-js-stable/set-timeout"));

var _concat = _interopRequireDefault(require("@babel/runtime-corejs3/core-js-stable/instance/concat"));

var _splitchangesRealWithSegments = _interopRequireDefault(require("../mocks/splitchanges.real.withSegments.json"));

var _splitchangesRealUpdateWithSegments = _interopRequireDefault(require("../mocks/splitchanges.real.updateWithSegments.json"));

var _splitchangesRealUpdateWithoutSegments = _interopRequireDefault(require("../mocks/splitchanges.real.updateWithoutSegments.json"));

var _mysegmentsNicolasSplitIo = _interopRequireDefault(require("../mocks/mysegments.nicolas@split.io.json"));

var _mysegmentsNicolasSplitIoMock = _interopRequireDefault(require("../mocks/mysegments.nicolas@split.io.mock2.json"));

var _mysegmentsMarcioSplitIo = _interopRequireDefault(require("../mocks/mysegments.marcio@split.io.json"));

var _messageOCCUPANCY0Control_pri = _interopRequireDefault(require("../mocks/message.OCCUPANCY.0.control_pri.1586987434550.json"));

var _messageOCCUPANCY1Control_pri = _interopRequireDefault(require("../mocks/message.OCCUPANCY.1.control_pri.1586987434450.json"));

var _messageOCCUPANCY1Control_sec = _interopRequireDefault(require("../mocks/message.OCCUPANCY.1.control_sec.1586987434451.json"));

var _messageOCCUPANCY2Control_pri = _interopRequireDefault(require("../mocks/message.OCCUPANCY.2.control_pri.1586987434650.json"));

var _messageCONTROLSTREAMING_PAUSEDControl_pri = _interopRequireDefault(require("../mocks/message.CONTROL.STREAMING_PAUSED.control_pri.1586987434750.json"));

var _messageCONTROLSTREAMING_RESUMEDControl_pri = _interopRequireDefault(require("../mocks/message.CONTROL.STREAMING_RESUMED.control_pri.1586987434850.json"));

var _messageCONTROLSTREAMING_DISABLEDControl_pri = _interopRequireDefault(require("../mocks/message.CONTROL.STREAMING_DISABLED.control_pri.1586987434950.json"));

var _messageSPLIT_UPDATE = _interopRequireDefault(require("../mocks/message.SPLIT_UPDATE.1457552649999.json"));

var _messageMY_SEGMENTS_UPDATENicolasSplitIo = _interopRequireDefault(require("../mocks/message.MY_SEGMENTS_UPDATE.nicolas@split.io.1457552640000.json"));

var _authPushEnabledNicolasSplitIo = _interopRequireDefault(require("../mocks/auth.pushEnabled.nicolas@split.io.json"));

var _authPushEnabledNicolasSplitIoMarcioSplitIo = _interopRequireDefault(require("../mocks/auth.pushEnabled.nicolas@split.io.marcio@split.io.json"));

var _utils = require("../utils");

var _eventSourceMock = _interopRequireWildcard(require("../../sync/__tests__/mocks/eventSourceMock"));

var _index = require("../../index");

var _settings = _interopRequireDefault(require("../../utils/settings"));

/**
 * Validate the handling of OCCUPANCY and CONTROL events
 */
// since: -1, till: 1457552620999 (for initial fetch)
// since: 1457552620999, till: 1457552649999 (for SPLIT_UPDATE event)
// since: 1457552649999, till: 1457552669999 (for second polling fetch)
window.EventSource = _eventSourceMock.default;
var userKey = 'nicolas@split.io';
var secondUserKey = 'marcio@split.io';
var thirdUserKey = 'facundo@split.io';
var baseUrls = {
  sdk: 'https://sdk.push-synchronization/api',
  events: 'https://events.push-synchronization/api',
  auth: 'https://auth.push-synchronization/api'
};
var config = {
  core: {
    authorizationKey: '<fake-token-push-1>',
    key: userKey
  },
  scheduler: {
    featuresRefreshRate: 0.2,
    segmentsRefreshRate: 0.25,
    metricsRefreshRate: 3000,
    impressionsRefreshRate: 3000
  },
  urls: baseUrls,
  streamingEnabled: true // debug: true,

};
var settings = (0, _settings.default)(config);
var MILLIS_SSE_OPEN = 100;
var MILLIS_STREAMING_DOWN_OCCUPANCY = MILLIS_SSE_OPEN + 100;
var MILLIS_SPLIT_UPDATE_EVENT_DURING_POLLING = MILLIS_STREAMING_DOWN_OCCUPANCY + 100;
var MILLIS_STREAMING_UP_OCCUPANCY = MILLIS_STREAMING_DOWN_OCCUPANCY + settings.scheduler.featuresRefreshRate + 100;
var MILLIS_CREATE_CLIENT_DURING_PUSH = MILLIS_STREAMING_UP_OCCUPANCY + 50;
var MILLIS_SPLIT_UPDATE_EVENT_DURING_PUSH = MILLIS_STREAMING_UP_OCCUPANCY + 100;
var MILLIS_STREAMING_PAUSED_CONTROL = MILLIS_SPLIT_UPDATE_EVENT_DURING_PUSH + 100;
var MILLIS_MY_SEGMENTS_UPDATE_EVENT_DURING_POLLING = MILLIS_STREAMING_PAUSED_CONTROL + 100;
var MILLIS_STREAMING_RESUMED_CONTROL = MILLIS_STREAMING_PAUSED_CONTROL + settings.scheduler.featuresRefreshRate + 100;
var MILLIS_MY_SEGMENTS_UPDATE_EVENT_DURING_PUSH = MILLIS_STREAMING_RESUMED_CONTROL + 100;
var MILLIS_STREAMING_DISABLED_CONTROL = MILLIS_MY_SEGMENTS_UPDATE_EVENT_DURING_PUSH + 100;
var MILLIS_DESTROY = MILLIS_STREAMING_DISABLED_CONTROL + settings.scheduler.featuresRefreshRate * 2 + 100;
/**
 * Sequence of calls:
 *  0.0 secs: initial SyncAll (/splitChanges, /mySegments/*), auth, SSE connection
 *  0.1 secs: SSE connection opened -> syncAll (/splitChanges, /mySegments/nicolas)
 *  0.2 secs: Streaming down (OCCUPANCY event) -> fetch due to fallback to polling (/splitChanges, /mySegments/nicolas)
 *  0.3 secs: SPLIT_UPDATE event ignored
 *  0.4 secs: periodic fetch due to polling (/splitChanges)
 *  0.45 secs: periodic fetch due to polling (/mySegments/*)
 *  0.5 secs: Streaming up (OCCUPANCY event) -> syncAll (/splitChanges, /mySegments/nicolas)
 *  0.55 secs: create a new client while streaming -> initial fetch (/mySegments/marcio), auth, SSE connection and syncAll (/splitChanges, /mySegments/nicolas, /mySegments/marcio)
 *  0.6 secs: SPLIT_UPDATE event -> /splitChanges
 *  0.7 secs: Streaming down (CONTROL event) -> fetch due to fallback to polling (/splitChanges, /mySegments/nicolas, /mySegments/marcio)
 *  0.7 secs: create a new client while polling -> initial fetch (/mySegments/facundo), auth fail (continue polling)
 *  0.8 secs: MY_SEGMENTS_UPDATE event ignored
 *  0.9 secs: periodic fetch due to polling (/splitChanges)
 *  0.95 secs: periodic fetch due to polling (/mySegments/nicolas, /mySegments/marcio, /mySegments/facundo)
 *  1.0 secs: Streaming up (CONTROL event) -> syncAll (/splitChanges, /mySegments/nicolas, /mySegments/marcio, /mySegments/facundo)
 *  1.1 secs: MY_SEGMENTS_UPDATE event -> /mySegments/nicolas
 *  1.2 secs: Streaming down (CONTROL event) -> fetch due to fallback to polling (/splitChanges, /mySegments/nicolas, /mySegments/marcio, /mySegments/facundo)
 *  1.4 secs: periodic fetch due to polling (/splitChanges): due to update without segments, mySegments are not fetched
 *  1.6 secs: periodic fetch due to polling (/splitChanges)
 *  1.7 secs: destroy client
 */

function testFallbacking(fetchMock, assert) {
  var _context3, _context4, _context5;

  assert.plan(16);
  fetchMock.reset();
  var start, splitio, client; // eslint-disable-next-line no-unused-vars

  var secondClient, thirdClient; // mock SSE open and message events

  (0, _eventSourceMock.setMockListener)(function (eventSourceInstance) {
    var _context;

    var expectedSSEurl = (0, _concat.default)(_context = "".concat(settings.url('/sse'), "?channels=NzM2MDI5Mzc0_NDEzMjQ1MzA0Nw%3D%3D_NTcwOTc3MDQx_mySegments,NzM2MDI5Mzc0_NDEzMjQ1MzA0Nw%3D%3D_splits,%5B%3Foccupancy%3Dmetrics.publishers%5Dcontrol_pri,%5B%3Foccupancy%3Dmetrics.publishers%5Dcontrol_sec&accessToken=")).call(_context, _authPushEnabledNicolasSplitIo.default.token, "&v=1.1&heartbeats=true");
    assert.equals(eventSourceInstance.url, expectedSSEurl, 'EventSource URL is the expected');
    (0, _setTimeout2.default)(function () {
      eventSourceInstance.emitOpen();
      eventSourceInstance.emitMessage(_messageOCCUPANCY1Control_pri.default);
      eventSourceInstance.emitMessage(_messageOCCUPANCY1Control_sec.default);
    }, MILLIS_SSE_OPEN); // open SSE connection after 0.1 seconds

    (0, _setTimeout2.default)(function () {
      eventSourceInstance.emitMessage(_messageOCCUPANCY0Control_pri.default);
    }, MILLIS_STREAMING_DOWN_OCCUPANCY); // send an OCCUPANCY event for switching to polling

    (0, _setTimeout2.default)(function () {
      eventSourceInstance.emitMessage(_messageSPLIT_UPDATE.default);
    }, MILLIS_SPLIT_UPDATE_EVENT_DURING_POLLING); // send a SPLIT_UPDATE event while polling, to check that we are ignoring it

    (0, _setTimeout2.default)(function () {
      eventSourceInstance.emitMessage(_messageOCCUPANCY2Control_pri.default);
    }, MILLIS_STREAMING_UP_OCCUPANCY); // send a OCCUPANCY event for switching to push

    (0, _setTimeout2.default)(function () {
      secondClient = splitio.client(secondUserKey);
      (0, _eventSourceMock.setMockListener)(function (eventSourceInstance) {
        var _context2;

        var expectedSSEurl = (0, _concat.default)(_context2 = "".concat(settings.url('/sse'), "?channels=NzM2MDI5Mzc0_NDEzMjQ1MzA0Nw%3D%3D_MjE0MTkxOTU2Mg%3D%3D_mySegments,NzM2MDI5Mzc0_NDEzMjQ1MzA0Nw%3D%3D_NTcwOTc3MDQx_mySegments,NzM2MDI5Mzc0_NDEzMjQ1MzA0Nw%3D%3D_splits,%5B%3Foccupancy%3Dmetrics.publishers%5Dcontrol_pri,%5B%3Foccupancy%3Dmetrics.publishers%5Dcontrol_sec&accessToken=")).call(_context2, _authPushEnabledNicolasSplitIoMarcioSplitIo.default.token, "&v=1.1&heartbeats=true");
        assert.equals(eventSourceInstance.url, expectedSSEurl, 'new EventSource URL is the expected');
        eventSourceInstance.emitOpen();
        (0, _setTimeout2.default)(function () {
          assert.equal(client.getTreatment('real_split'), 'on', 'evaluation of initial Split');
          client.once(client.Event.SDK_UPDATE, function () {
            assert.equal(client.getTreatment('real_split'), 'off', 'evaluation of updated Split');
          });
          eventSourceInstance.emitMessage(_messageSPLIT_UPDATE.default);
        }, MILLIS_SPLIT_UPDATE_EVENT_DURING_PUSH - MILLIS_CREATE_CLIENT_DURING_PUSH); // send a SPLIT_UPDATE event when push resumed, to check that we are handling it

        (0, _setTimeout2.default)(function () {
          eventSourceInstance.emitMessage(_messageCONTROLSTREAMING_PAUSEDControl_pri.default);
          thirdClient = splitio.client(thirdUserKey);
        }, MILLIS_STREAMING_PAUSED_CONTROL - MILLIS_CREATE_CLIENT_DURING_PUSH); // send a CONTROL event for switching to polling

        (0, _setTimeout2.default)(function () {
          eventSourceInstance.emitMessage(_messageMY_SEGMENTS_UPDATENicolasSplitIo.default);
        }, MILLIS_MY_SEGMENTS_UPDATE_EVENT_DURING_POLLING - MILLIS_CREATE_CLIENT_DURING_PUSH); // send a MY_SEGMENTS_UPDATE event while polling, to check that we are ignoring it

        (0, _setTimeout2.default)(function () {
          eventSourceInstance.emitMessage(_messageCONTROLSTREAMING_RESUMEDControl_pri.default);
        }, MILLIS_STREAMING_RESUMED_CONTROL - MILLIS_CREATE_CLIENT_DURING_PUSH); // send a CONTROL event for switching to push

        (0, _setTimeout2.default)(function () {
          assert.equal(client.getTreatment('real_split'), 'off', 'evaluation with initial segment');
          client.once(client.Event.SDK_UPDATE, function () {
            assert.equal(client.getTreatment('real_split'), 'on', 'evaluation with updated segment');
          });
          eventSourceInstance.emitMessage(_messageMY_SEGMENTS_UPDATENicolasSplitIo.default);
        }, MILLIS_MY_SEGMENTS_UPDATE_EVENT_DURING_PUSH - MILLIS_CREATE_CLIENT_DURING_PUSH); // send a MY_SEGMENTS_UPDATE event

        (0, _setTimeout2.default)(function () {
          eventSourceInstance.emitMessage(_messageCONTROLSTREAMING_DISABLEDControl_pri.default);
          assert.equal(eventSourceInstance.readyState, _eventSourceMock.default.CLOSED, 'EventSource connection closed on STREAMING_DISABLED CONTROL event');
        }, MILLIS_STREAMING_DISABLED_CONTROL - MILLIS_CREATE_CLIENT_DURING_PUSH); // send a CONTROL event for disabling push and switching to polling

        (0, _setTimeout2.default)(function () {
          client.destroy().then(function () {
            assert.pass('client destroyed');
          });
        }, MILLIS_DESTROY - MILLIS_CREATE_CLIENT_DURING_PUSH); // destroy client after 1.7 seconds
      });
    }, MILLIS_CREATE_CLIENT_DURING_PUSH); // create a second client
  });
  fetchMock.getOnce(settings.url("/auth?users=".concat(encodeURIComponent(userKey))), function (url, opts) {
    if (!opts.headers['Authorization']) assert.fail('`/auth` request must include `Authorization` header');
    assert.pass('auth success');
    return {
      status: 200,
      body: _authPushEnabledNicolasSplitIo.default
    };
  }); // initial split and mySegment sync

  fetchMock.getOnce(settings.url('/splitChanges?since=-1'), {
    status: 200,
    body: _splitchangesRealWithSegments.default
  });
  fetchMock.getOnce(settings.url('/mySegments/nicolas@split.io'), {
    status: 200,
    body: _mysegmentsNicolasSplitIo.default
  }); // split and segment sync after SSE opened

  fetchMock.getOnce(settings.url('/splitChanges?since=1457552620999'), {
    status: 200,
    body: {
      splits: [],
      since: 1457552620999,
      till: 1457552620999
    }
  });
  fetchMock.getOnce(settings.url('/mySegments/nicolas@split.io'), {
    status: 200,
    body: _mysegmentsNicolasSplitIo.default
  }); // fetches due to first fallback to polling

  fetchMock.getOnce(settings.url('/splitChanges?since=1457552620999'), {
    status: 200,
    body: {
      splits: [],
      since: 1457552620999,
      till: 1457552620999
    }
  });
  fetchMock.getOnce(settings.url('/mySegments/nicolas@split.io'), {
    status: 200,
    body: _mysegmentsNicolasSplitIo.default
  });
  fetchMock.getOnce(settings.url('/splitChanges?since=1457552620999'), function () {
    var lapse = (0, _now.default)() - start;
    assert.true((0, _utils.nearlyEqual)(lapse, MILLIS_STREAMING_DOWN_OCCUPANCY + settings.scheduler.featuresRefreshRate), 'fetch due to first fallback to polling');
    return {
      status: 200,
      body: {
        splits: [],
        since: 1457552620999,
        till: 1457552620999
      }
    };
  });
  fetchMock.getOnce(settings.url('/mySegments/nicolas@split.io'), {
    status: 200,
    body: _mysegmentsNicolasSplitIo.default
  }); // split and segment sync due to streaming up (OCCUPANCY event)

  fetchMock.getOnce(settings.url('/splitChanges?since=1457552620999'), {
    status: 200,
    body: {
      splits: [],
      since: 1457552620999,
      till: 1457552620999
    }
  });
  fetchMock.getOnce(settings.url('/mySegments/nicolas@split.io'), {
    status: 200,
    body: _mysegmentsNicolasSplitIo.default
  }); // creating of second client during streaming: initial mysegment sync, reauth and syncAll due to new client

  fetchMock.getOnce(settings.url('/mySegments/marcio@split.io'), {
    status: 200,
    body: _mysegmentsMarcioSplitIo.default
  });
  fetchMock.getOnce(settings.url((0, _concat.default)(_context3 = "/auth?users=".concat(encodeURIComponent(userKey), "&users=")).call(_context3, encodeURIComponent(secondUserKey))), function (url, opts) {
    if (!opts.headers['Authorization']) assert.fail('`/auth` request must include `Authorization` header');
    assert.pass('second auth success');
    return {
      status: 200,
      body: _authPushEnabledNicolasSplitIoMarcioSplitIo.default
    };
  });
  fetchMock.getOnce(settings.url('/splitChanges?since=1457552620999'), {
    status: 200,
    body: {
      splits: [],
      since: 1457552620999,
      till: 1457552620999
    }
  });
  fetchMock.getOnce(settings.url('/mySegments/nicolas@split.io'), {
    status: 200,
    body: _mysegmentsNicolasSplitIo.default
  });
  fetchMock.getOnce(settings.url('/mySegments/marcio@split.io'), {
    status: 200,
    body: _mysegmentsMarcioSplitIo.default
  }); // fetch due to SPLIT_UPDATE event

  fetchMock.getOnce(settings.url('/splitChanges?since=1457552620999'), function () {
    var lapse = (0, _now.default)() - start;
    assert.true((0, _utils.nearlyEqual)(lapse, MILLIS_SPLIT_UPDATE_EVENT_DURING_PUSH), 'sync due to SPLIT_UPDATE event');
    return {
      status: 200,
      body: _splitchangesRealUpdateWithSegments.default
    };
  }); // fetches due to second fallback to polling

  fetchMock.getOnce(settings.url('/splitChanges?since=1457552649999'), {
    status: 200,
    body: {
      splits: [],
      since: 1457552649999,
      till: 1457552649999
    }
  });
  fetchMock.getOnce(settings.url('/mySegments/nicolas@split.io'), {
    status: 200,
    body: _mysegmentsNicolasSplitIo.default
  });
  fetchMock.getOnce(settings.url('/mySegments/marcio@split.io'), {
    status: 200,
    body: _mysegmentsMarcioSplitIo.default
  }); // creation of third client during polling: initial mysegment sync and authentication

  fetchMock.getOnce(settings.url('/mySegments/facundo@split.io'), {
    status: 200,
    body: _mysegmentsMarcioSplitIo.default
  }); // authentication fail, so we keep polling. next auth attempt is scheduled in one second (after the test finishes)

  fetchMock.getOnce(settings.url((0, _concat.default)(_context4 = (0, _concat.default)(_context5 = "/auth?users=".concat(encodeURIComponent(userKey), "&users=")).call(_context5, encodeURIComponent(secondUserKey), "&users=")).call(_context4, encodeURIComponent(thirdUserKey))), {
    throws: new TypeError('Network error')
  }); // continue fetches due to second fallback to polling

  fetchMock.getOnce(settings.url('/splitChanges?since=1457552649999'), function () {
    var lapse = (0, _now.default)() - start;
    assert.true((0, _utils.nearlyEqual)(lapse, MILLIS_STREAMING_PAUSED_CONTROL + settings.scheduler.featuresRefreshRate), 'fetch due to second fallback to polling');
    return {
      status: 200,
      body: {
        splits: [],
        since: 1457552649999,
        till: 1457552649999
      }
    };
  });
  fetchMock.getOnce(settings.url('/mySegments/nicolas@split.io'), {
    status: 200,
    body: _mysegmentsNicolasSplitIo.default
  });
  fetchMock.getOnce(settings.url('/mySegments/marcio@split.io'), {
    status: 200,
    body: _mysegmentsMarcioSplitIo.default
  });
  fetchMock.getOnce(settings.url('/mySegments/facundo@split.io'), {
    status: 200,
    body: _mysegmentsMarcioSplitIo.default
  }); // split and segment sync due to streaming up (CONTROL event)

  fetchMock.getOnce(settings.url('/splitChanges?since=1457552649999'), {
    status: 200,
    body: {
      splits: [],
      since: 1457552649999,
      till: 1457552649999
    }
  });
  fetchMock.getOnce(settings.url('/mySegments/nicolas@split.io'), {
    status: 200,
    body: _mysegmentsNicolasSplitIo.default
  });
  fetchMock.getOnce(settings.url('/mySegments/marcio@split.io'), {
    status: 200,
    body: _mysegmentsMarcioSplitIo.default
  });
  fetchMock.getOnce(settings.url('/mySegments/facundo@split.io'), {
    status: 200,
    body: _mysegmentsMarcioSplitIo.default
  }); // fetch due to MY_SEGMENTS_UPDATE event

  fetchMock.getOnce(settings.url('/mySegments/nicolas@split.io'), function () {
    var lapse = (0, _now.default)() - start;
    assert.true((0, _utils.nearlyEqual)(lapse, MILLIS_MY_SEGMENTS_UPDATE_EVENT_DURING_PUSH), 'sync due to MY_SEGMENTS_UPDATE event');
    return {
      status: 200,
      body: _mysegmentsNicolasSplitIoMock.default
    };
  }); // fetches due to third fallback to polling (mySegments is not fetched after the first iteration)

  fetchMock.getOnce(settings.url('/splitChanges?since=1457552649999'), {
    status: 200,
    body: {
      splits: [],
      since: 1457552649999,
      till: 1457552649999
    }
  });
  fetchMock.getOnce(settings.url('/mySegments/nicolas@split.io'), {
    status: 200,
    body: _mysegmentsNicolasSplitIo.default
  });
  fetchMock.getOnce(settings.url('/mySegments/marcio@split.io'), {
    status: 200,
    body: _mysegmentsMarcioSplitIo.default
  });
  fetchMock.getOnce(settings.url('/mySegments/facundo@split.io'), {
    status: 200,
    body: _mysegmentsMarcioSplitIo.default
  });
  fetchMock.getOnce(settings.url('/splitChanges?since=1457552649999'), function () {
    var lapse = (0, _now.default)() - start;
    assert.true((0, _utils.nearlyEqual)(lapse, MILLIS_STREAMING_DISABLED_CONTROL + settings.scheduler.featuresRefreshRate), 'fetch due to third fallback to polling');
    return {
      status: 200,
      body: _splitchangesRealUpdateWithoutSegments.default
    };
  });
  fetchMock.getOnce(settings.url('/splitChanges?since=1457552669999'), function () {
    var lapse = (0, _now.default)() - start;
    assert.true((0, _utils.nearlyEqual)(lapse, MILLIS_STREAMING_DISABLED_CONTROL + settings.scheduler.featuresRefreshRate * 2), 'fetch due to third fallback to polling');
    return {
      status: 200,
      body: {
        splits: [],
        since: 1457552669999,
        till: 1457552669999
      }
    };
  });
  fetchMock.get(new RegExp('.*'), function (url) {
    assert.fail('unexpected GET request with url: ' + url);
  });
  fetchMock.post('*', 200);
  start = (0, _now.default)();
  splitio = (0, _index.SplitFactory)(config);
  client = splitio.client();
}