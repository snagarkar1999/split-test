...
/* undefined for node */
  ...
  function scheduleTokenRefresh(issuedAt, expirationTime) {
    ...
    timeoutId = _setTimeout(connectPush, delayInSeconds * 1000);
  }

  function connectPush() {
    log.info('Connecting to push streaming.');
    var userKeys = clientContexts ? _Object$keys(clientContexts) : undefined;
    authenticate(settings, userKeys).then(function (authData) {
      ...
      if (userKeys && userKeys.length < _Object$keys(clientContexts).length) return; // Connect to SSE and schedule refresh token
      ...
      log.error(_concatInstanceProperty(_context = "Fail to authenticate for push notifications, with error message: \"".concat(error.message, "\". Attempting to reauthenticate in ")).call(_context, delayInMillis / 1000, " seconds."));
    });
  ...
  function stopWorkers() {
    _forEachInstanceProperty(workers).call(workers, function (worker) {
  ...
  pushEmitter.on(SSE_ERROR, function (error) {
    ...
    log.error(_concatInstanceProperty(_context2 = "Fail to connect to streaming".concat(errorMessage ? ", with error message: \"".concat(errorMessage, "\"") : '', ". Attempting to reconnect in ")).call(_context2, delayInMillis / 1000, " seconds."));
    pushEmitter.emit(PUSH_DISCONNECT); // no harm if polling already
  });
  ...
  return _Object$assign( // Expose Event Emitter functionality and Event constants
  _Object$create(pushEmitter), {
    ...
    startNewClient: function startNewClient(userKey, context) {
      ...
      _setTimeout(function checkForReconnect() {
