...
tape('SSEHandler', function (t) {
  ...
  t.test('`handlerMessage` for update notifications (NotificationProcessor)', function (assert) {
    ...
    assert["true"]((_pushEmitter$emit$las = pushEmitter.emit.lastCall).calledWithExactly.apply(_pushEmitter$emit$las, _concatInstanceProperty(_context = [SPLIT_UPDATE]).call(_context, _toConsumableArray(expectedParams))), 'must emit SPLIT_UPDATE with the message change number');
    expectedParams = [1457552650000, 'whitelist', 'not_allowed'];
    sseHandler.handleMessage(splitKillMessage);
    assert["true"]((_pushEmitter$emit$las2 = pushEmitter.emit.lastCall).calledWithExactly.apply(_pushEmitter$emit$las2, _concatInstanceProperty(_context2 = [SPLIT_KILL]).call(_context2, _toConsumableArray(expectedParams))), 'must emit SPLIT_KILL with the message change number, split name and default treatment');
    expectedParams = [1457552640000, 'splitters'];
    sseHandler.handleMessage(segmentUpdateMessage);
    assert["true"]((_pushEmitter$emit$las3 = pushEmitter.emit.lastCall).calledWithExactly.apply(_pushEmitter$emit$las3, _concatInstanceProperty(_context3 = [SEGMENT_UPDATE]).call(_context3, _toConsumableArray(expectedParams))), 'must emit SEGMENT_UPDATE with the message change number and segment name');
    expectedParams = [{
    ...
    assert["true"]((_pushEmitter$emit$las4 = pushEmitter.emit.lastCall).calledWithExactly.apply(_pushEmitter$emit$las4, _concatInstanceProperty(_context4 = [MY_SEGMENTS_UPDATE]).call(_context4, _toConsumableArray(expectedParams))), 'must emit MY_SEGMENTS_UPDATE with the message parsed data and channel');
    assert.end();
  });
