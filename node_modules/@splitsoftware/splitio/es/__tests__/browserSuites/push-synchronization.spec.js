...
import { nearlyEqual } from '../utils'; // Replace original EventSource with mock
...
export function testSynchronization(fetchMock, assert) {
  ...
  fetchMock.getOnce(settings.url(_concatInstanceProperty(_context3 = "/auth?users=".concat(encodeURIComponent(userKey), "&users=")).call(_context3, encodeURIComponent(otherUserKey))), function (url, opts) {
    if (!opts.headers['Authorization']) assert.fail('`/auth` request must include `Authorization` header');
