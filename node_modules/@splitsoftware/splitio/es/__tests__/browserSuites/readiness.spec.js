...
export default function (fetchMock, assert) {
  ...
  function mockForSegmentsPauseTest(testUrls) {
    ...
    fetchMock.get(new RegExp("".concat(testUrls.sdk, "/mySegments/nicolas\\d?@split.io")), function () {
    ...
    if (startWithSegments) {
      ...
      fetchMock.get(testUrls.sdk + '/splitChanges?since=1457552620999', {
        status: 200,
        body: _objectSpread({}, splitChangesUpdateWithoutSegmentsMock, {
          since: 1457552620999,
          till: 1457552649999
        })
      });
      fetchMock.get(testUrls.sdk + '/splitChanges?since=1457552649999', {
        status: 200,
        body: _objectSpread({}, splitChangesUpdateWithSegmentsMock, {
          since: 1457552649999,
          till: 1457552669999
        })
      });
  ...
  assert.test(function (t) {
    ...
    client.once(client.Event.SDK_READY, function () {
      t.ok(_Date$now() - start < 50, 'It should be ready really quickly, without waiting for mySegments, as there were no segments in the first splits payload.');
          ...
          client.once(client.Event.SDK_UPDATE, function () {
            _setTimeout(function () {
              ...
              _setTimeout(function () {
                ...
                _Promise.all([client2.destroy(), client3.destroy(), client.destroy()]).then(function () {
                  t.end();
                });
              }, 10000);
            }, 0);
  ...
  assert.test(function (t) {
    ...
    var client3 = splitio.client('nicolas3@split.io');
    client.once(client.Event.SDK_READY, function () {
      t.ok(_Date$now() - start >= mySegmentsEndpointDelay, 'It should not be ready without waiting for mySegments, as there are segments in the first splits payload.');
  ...
  assert.test(function (t) {
    ...
    var client3 = splitio.client('nicolas3@split.io');
    client.once(client.Event.SDK_READY, function () {
      t.ok(_Date$now() - start >= mySegmentsEndpointDelay, 'It should not be ready without waiting for mySegments, when we start from cache it might be stale.');
  ...
  assert.test(function (t) {
    ...
    var client3 = splitio.client('nicolas3@split.io');
    client.once(client.Event.SDK_READY, function () {
      t.ok(_Date$now() - start < 50, 'It should be ready quickly, since it had no segments and update has no segments either.');
  ...
  assert.test(function (t) {
    ...
    var client3 = splitio.client('nicolas3@split.io');
    client.once(client.Event.SDK_READY, function () {
      t.ok(_Date$now() - start >= mySegmentsEndpointDelay, 'It should not be ready without waiting for mySegments, when we start from cache it might be stale and we had segments even though the update has nothing.');
  ...
  assert.test(function (t) {
    ...
    var client3 = splitio.client('nicolas3@split.io');
    client.once(client.Event.SDK_READY, function () {
      t.ok(_Date$now() - start < 50, 'It should be ready without waiting for mySegments, since when it downloads changes it will have no more use for them.');
