...
tape('defaultMapper', function (assert) {
  ...
  assert["true"](initTimestamp <= defaultEvent.timestamp && defaultEvent.timestamp <= _Date$now());
  assert.end();
});
var sdkOptions = {
...
tape('GaToSplit', function (assert) {
  ...
  assert.deepEqual(event, _objectSpread({}, customMapper(modelSample, defaultMapper(modelSample)), {
  ...
  window.ga('send', hitSample);
  event = fakeStorage.events.track.lastCall.args[0];
  assert.deepEqual(event, _objectSpread({}, customMapper2(modelSample, defaultMapper(modelSample)), {
    key: customIdentities[0].key,
    trafficTypeName: customIdentities[0].trafficType,
    timestamp: event.timestamp
