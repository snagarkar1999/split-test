...
var SplitToGa = /*#__PURE__*/function () {
  ...
  function SplitToGa(options) {
    ...
    if (options) {
      if (typeof _filterInstanceProperty(options) === 'function') this.filter = _filterInstanceProperty(options);
      ...
      if (_Array$isArray(options.trackerNames)) this.trackerNames = uniq(options.trackerNames); // No need to validate `impressions` and `events` flags. Any other value than `false` is ignored.
    ...
    value: function queue(data) {
      ...
      if (ga) {
        ...
        try {
          ...
          // filter
          if (_filterInstanceProperty(this) && !_filterInstanceProperty(_context = this).call(_context, data)) return; // map data into a FieldsObject instance
        ...
        _forEachInstanceProperty(_context2 = this.trackerNames).call(_context2, function (trackerName) {
