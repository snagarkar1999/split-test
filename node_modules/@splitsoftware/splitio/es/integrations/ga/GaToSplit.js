...
var log = logFactory('splitio-ga-to-split');
/**
...
export function validateIdentities(identities) {
  if (!_Array$isArray(identities)) return []; // Remove duplicated identities

  var uniqueIdentities = unicAsStrings(identities); // Filter based on rum-agent identities validator

  return _filterInstanceProperty(uniqueIdentities).call(uniqueIdentities, function (identity) {
    if (!identity) return false;
    var maybeKey = identity.key;
    var maybeTT = identity.trafficType;
    if (!isString(maybeKey) && !isFinite(maybeKey)) return false;
...
export function validateEventData(eventData) {
  if (!validateEvent(eventData.eventTypeId, 'splitio-ga-to-split:mapper')) return false;
  if (validateEventValue(eventData.value, 'splitio-ga-to-split:mapper') === false) return false;

  var _validateEventPropert = validateEventProperties(eventData.properties, 'splitio-ga-to-split:mapper'),
      properties = _validateEventPropert.properties;

  if (properties === false) return false;
  if (eventData.timestamp && !isFinite(eventData.timestamp)) return false;
  if (eventData.key && validateKey(eventData.key, 'splitio-ga-to-split:mapper') === false) return false;
  if (eventData.trafficTypeName && validateTrafficType(eventData.trafficTypeName, 'splitio-ga-to-split:mapper') === false) return false;
  return true;
}
...
export function fixEventTypeId(eventTypeId) {
  ...
  var truncated = _sliceInstanceProperty(fixed).call(fixed, 0, 80);
...
function GaToSplit(sdkOptions, storage, coreSettings) {
  ...
  function SplitTracker(tracker, pluginOptions) {
    ...
    var opts = _Object$assign({}, defaultOptions, sdkOptions, pluginOptions);
    ...
    tracker.set('sendHitTask', function (model) {
      ...
      try {
        if (_filterInstanceProperty(opts) && !_filterInstanceProperty(opts).call(opts, model)) return;
      } catch (err) {
      ...
      if (opts.prefix) eventData.eventTypeId = _concatInstanceProperty(_context = "".concat(opts.prefix, ".")).call(_context, eventData.eventTypeId);
      ...
      if (eventData.key && eventData.trafficTypeName) {
        ...
        _forEachInstanceProperty(_context2 = opts.identities).call(_context2, function (identity) {
