...
function greedyFetch(settings, lastSinceValue, segmentName, metricCollectors) {
  return tracker.start(tracker.TaskNames.SEGMENTS_FETCH, metricCollectors, segmentChangesService(segmentChangesRequest(settings, {
    ...
    if (since === till) {
      ...
      return _Promise.all([json, greedyFetch(settings, till, segmentName)]).then(function (flatMe) {
        var _context;

        return _concatInstanceProperty(_context = [flatMe[0]]).call(_context, _toConsumableArray(flatMe[1]));
      });
    }
  })["catch"](function (err) {
